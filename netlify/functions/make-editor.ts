...
const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // Kullanıcı zaten editör mü kontrol et
    if (user.role === 'editor') {
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ 
          message: `${email} kullanıcısı zaten editör rolüne sahip.`,
          user: {
            id: user.id,
    ...
    return {
      ...
      headers,
      body: JSON.stringify({ 
        message: `${email} kullanıcısı başarıyla editör yapıldı.`,
        user: updatedUser
      }),
    };
  } catch (error) {
